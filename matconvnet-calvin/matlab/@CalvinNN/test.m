...
statsPath = fullfile(obj.nnOpts.expDir, sprintf('stats-%s-epoch%d.mat', 'test', epoch));
if exist(statsPath, 'file')
    % Load stats
    statsTest = load(statsPath);
else
    % Create stats
    ...
    % Process the epoch
    statsTest = obj.processEpoch(obj.net, state);
    save(statsPath, '-struct', 'statsTest');
end