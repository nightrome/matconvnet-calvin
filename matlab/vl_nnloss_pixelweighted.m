function Y = vl_nnloss_pixelweighted(X,c,dzdy,varargin)
...
opts.classWeights = [] ;
opts.pixelWeights = []; % Holger: weight each pixel in an image
opts.threshold = 0 ;
...
if nargin <= 2 || isempty(dzdy)
  ...
  % Holger weight loss
  if ~isempty(opts.pixelWeights),
      t = t .* opts.pixelWeights;
  end;
  ...
  % Holger weight gradients
  if ~isempty(opts.pixelWeights),
      dzdy = dzdy .* opts.pixelWeights;
  end;
